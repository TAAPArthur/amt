...
class MediaReader:
    ...
    def bundle_unread_chapters(self, name=None, shuffle=False, limit=None, ignore_errors=False):
        ...
        self.for_each(self._download_selected_chapters, self.get_unreads(MANGA, name=name, shuffle=shuffle, limit=limit), raiseException=not ignore_errors)
        ...
        for media_data in self.get_media():
            if not media_type or media_data["media_type"] == media_type:
                ...
                if tracker_info and (force or media_data["progress"] < int(self.get_last_read(media_data))):
                    data.append((tracker_info[0], self.get_last_read(media_data), media_data["progressVolumes"]))
                    last_read = self.get_last_read(media_data)
