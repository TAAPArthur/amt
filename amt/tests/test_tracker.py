...
class TestTracker(Tracker):
    ...
    def get_tracker_list(self, user_name=None, id=None, status="CURRENT"):
        return [self.get_media_dict(id=i, media_type=item[0], name=item[1], progress=item[2], score=item[3], year=i, year_end=i, external_links=item[4], streaming_links=item[5], nextTimeStamp=item[6]) for i, item in enumerate(self.media_list)] if self.customList is None else self.customList
