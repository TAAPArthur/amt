...
class TestTracker(Tracker):
    ...
    def __init__(self, session, settings=None):
        ...
        for media_type_name in MEDIA_TYPES:
            self.media_list.extend([
                [MEDIA_TYPES[media_type_name], f"{media_type_name}1", 0],
                [MEDIA_TYPES[media_type_name], f"{media_type_name}Unknown", 1],
                [MEDIA_TYPES[media_type_name], f"{media_type_name}InProgress", 1],
                [MEDIA_TYPES[media_type_name], f"{media_type_name}2", 0]
            ])
    ...
    def get_tracker_list(self, user_name=None, id=None, status="CURRENT"):
        return [self.get_media_dict(id=i, media_type=item[0], name=item[1], progress=item[2]) for i, item in enumerate(self.media_list)] if not self.customList else self.customList
